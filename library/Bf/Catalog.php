...
class Bf_Catalog {
	...
	*/
	...
	public function save($arrForms, $arrData) {
		...
		if ( empty($arrData['catalog_ids']) ) {
			if ( empty( $arrData[Bf_Catalog_Models_Db_Catalog::COL_ID_CATALOG] ) ) {
				$arrCatalogIds = array ("");
			} else {
				$arrCatalogIds = array( $arrData[Bf_Catalog_Models_Db_Catalog::COL_ID_CATALOG] );
			}
		} else {
			$arrCatalogIds = explode(',', $arrData['catalog_ids']);

			foreach ($arrForms as $objForm) {
				$arrElements = $objForm->getElements();
				foreach($arrElements as $objElement) {
					$objElement->clearValidators();
					$objElement->setRequired(FALSE);
				}
			}
		}
		...
		foreach ( $arrCatalogIds as $intCatalogId ) {
			if ( empty($intCatalogId) ) {
				$arrMergedData = $arrData;
			} else {
				$arrCurrentIdRowData = $this->getItemArray($intCatalogId);
				$arrItem = $this->getItemArray($intCatalogId);

				$objEavForm =
				$this
				->getObjEav()
				->getForm( $arrData[Bf_Eav_Db_EntitiesTypes::COL_ID_ENTITIES_TYPES], $this->getCatalogModel()->getObjCatalogData()->getLanguage(), $arrCurrentIdRowData[Bf_Catalog_Models_Db_Catalog::COL_ID_ENTITIES] );

				$arrEavArray = array();
				$arrElements = $objEavForm->getElements();
				foreach ($arrElements as $objElement ){
					$arrEavArray[$objElement->getName()] = $objElement->getValue();
				}

				$arrData = array_filter( $arrData, array( $this, '_filter_empty_values' ) );
				$arrMergedData = array_merge($arrEavArray, $arrItem, $arrCurrentIdRowData, $arrData);
			}

			if ( ! $this->isValidForms($arrForms, $arrMergedData) ) {
				continue;
			}
			...
			if ( ! empty( $arrForms[ $this->getObjEav()->getOptions('formName') ] ) ) {
				...
				$intEntityId = $this->getObjEav()->saveForm( $arrCleanCatalogData[Bf_Catalog_Models_Db_Catalog::COL_ID_ENTITIES], $objEavForm, $this->getCatalogModel()->getObjCatalogData()->getLanguage() );
			...
			if ( $this->getCatalogModel()->getObjCatalogData()->save($intCatId, $objDataForm) ) {
				$intReturnId = $intCatId;
			}
		}
	...
	public function delete($intCatId) {
		...
		$objCatTableSubSelect->columns( array (
			'childrens' => new Zend_Db_Expr ( 'count(*)' )
		) );
		...
		$objCatTableSelect->columns( array (
			'childrens' => new Zend_Db_Expr ( "(" . $objCatTableSubSelect . ")" )
		) );
	...
	* @param bool $boolGetSelect
	*        	default TRUE
	*/
	public function getItems($intParent = 0, $boolGetSelect = TRUE, $boolFoldersTreeOnly = FALSE, $boolItemsListOnly = FALSE, $boolGetBranch = FALSE) {
		if ($boolGetSelect) {
			return $this->objCatalogModel->getCatalogSelectByParent( $intParent, $boolFoldersTreeOnly, $boolItemsListOnly, $boolGetBranch );
		} else {
			return $this->objCatalogModel->getCatalogByParent( $intParent, $boolFoldersTreeOnly, $boolItemsListOnly, $boolGetBranch );
		}
	}
	...
	*/
	public function getItemArray($intCatalogId) {
		...
		if (! empty ( $objItem->{Bf_Catalog_Models_Db_Catalog::COL_ID_ENTITIES} )) {
			...
			// $arrItem[$this->getStrEavIndex()] =
			// $objEav->getEntityData($objItem->{Bf_Catalog_Models_Db_Catalog::COL_ID_ENTITIES},$this->getCatalogModel()->getObjCatalogData()->getLanguage());
		}
	...
	*/
	private function getForms($intEntTypeId, Array $arrValues, $boolIsFolder) {
		...
		if ( ( $boolIsFolder && $this->getOptions('useFolderEav') ) || ( ! $boolIsFolder && $this->getOptions('useItemEav') ) ) {
			...
			$objEavForm = $this->getObjEav()->getForm( $intEntTypeId, $this->getCatalogModel()->getObjCatalogData()->getLanguage(), $intEntityId );
	...
	private function _filter_empty_values($value) {
