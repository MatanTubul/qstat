	...
	*/
	public function saveForm ($intEntityId, Zend_Form $objForm, $intLangId)
	{
		...
		foreach ($objAttributes as $objAttribute) {
			...
			$mixValue = FALSE;
			$objFormElement = $objForm->getElement($objForm->filterName($objAttribute->{Bf_Eav_Db_Attributes::COL_ATTR_CODE}));
			if (!empty($objFormElement)){
				$mixValue =  $objFormElement->getValue();
			}

			if (! empty($objValue) && $mixValue !== FALSE) {
				$objElement = $objValue->saveElement($intEntityId, $objAttribute->{Bf_Eav_Db_Attributes::COL_ID_ATTR}, $mixValue, $intLangId);
			}
		}
	...
	protected function getGroupAttributes($intGroupId)
	{
		...
		$objSelect->where(Bf_Eav_Db_GroupAttributes::TBL_NAME . '.' . Bf_Eav_Db_GroupAttributes::COL_IS_DELETED . " = ?", 0);
		$objSelect->where(Bf_Eav_Db_Attributes::TBL_NAME . '.' . Bf_Eav_Db_Attributes::COL_IS_DELETED . " = ?", 0);
	...
	protected function getGroupsByEntityType($intEntityTypeId)
	{
		...
		$objSelect->where(Bf_Eav_Db_EntitiesTypesGroups::TBL_NAME . '.' . Bf_Eav_Db_EntitiesTypesGroups::COL_IS_DELETED . " = ?", 0);
		$objSelect->order(Bf_Eav_Db_EntitiesTypesGroups::COL_ORDER);

		return $objGroups->fetchAll($objSelect);
	}

	public function getAttributeByEntityType($intEntityTypeId)
	{
