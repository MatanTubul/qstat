...
class Bf_Service_Cron {
	...
	*/
	...
	{
		// Initialize the log before we fork.
		// That way child processes will have a reference to the same log as the parent process.
		$log = $this->getLog();
		$children = array();
		foreach ($this->_actions as $key => $action) {
			...
			// Check to see if this task is locked (currently running, probably due to an earlier cron run).
			...
			if ($pid == -1) {
				...
				try {
					...
					if (APPLICATION_ENV == 'development') {
						$log->err('[' . $mypid . '] [DEV]: '.PHP_EOL.$e->getTraceAsString().PHP_EOL.$e->getMessage());
					} else {
		...
		// Now that we've started all the actions, we just need to wait for them to finish and clean everything up.
		// The following gets rid of the zombie processes leftover when the child processes die.
		foreach ($children as $child) {
			pcntl_waitpid($child, $status);
		}

		// At this point all the child processes should be finished. We can output the log.
		// Save a copy as "cron.latest.log" so we can look it over if necessary,
		// but in general if there's any output it'll be emailed to the cron runner user anyway.
