...
abstract class Bf_Eav_Value_Multi extends Bf_Eav_Value_Simple
{
	...
	public function getAttrEditExtra ($intAttrId, $mixAttrType) {
		...
		$strUrl = $viewRenderer->view->url(
			array(
				'module' => 'eav',
				'controller' => 'index',
				'action' => 'save-attrib-multi-value',
				Bf_Eav_Db_Attributes::COL_ID_ATTR => $intAttrId,
				Bf_Eav_Db_Attributes::COL_VALUE_TYPE => $mixAttrType,
			),
			null,
			true
		);
		...
		$objGridPager->setDefaultAdd(true);
		$objGridPager->setDefaultEdit(true);
		$objGridPager->setDefaultDel(true);
	...
	public function setGridCol ($objAttrListRow, $objGrid,$strSearchValue="")
	{
		...
		$objColumn = new Ingot_JQuery_JqGrid_Column(
			$objForm->filterName( $objAttrListRow->{Bf_Eav_Db_Attributes::COL_ATTR_CODE} ),
			array(
				'autowidth' => true,
				'shrinkToFit' => false,
				'sortable' => false,
		...
		$arrValuesPairs = self::getValuePairs( array(), $objAttrListRow->{Bf_Eav_Db_Attributes::COL_ID_ATTR} );
		if ( ! empty($strSearchValue) ) {
			foreach ( $arrValuesPairs as $intId => $strValue ) {
				if ( $strSearchValue == $strValue ) {
					$strSearchValue = $intId;
				}
			}
		}

		$objParentDecorator = new Ingot_JQuery_JqGrid_Column_Decorator_Search_Select(
			$objColumn,
			array(
				'value' => $arrValuesPairs,
				'sopt' => array('bw'),
				'defaultValue' => $strSearchValue,
			)
		);

		$objDecorator = new Ingot_JQuery_JqGrid_Column_Decorator_EavSelect(
			$objParentDecorator,
			array(
