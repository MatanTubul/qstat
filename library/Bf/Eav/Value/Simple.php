...
abstract class Bf_Eav_Value_Simple extends Bf_Eav_Value_Abstract
{
	...
	public static function saveElementHelper ($strValueTableClass, $intEntityId, $intAttribId, $mixValue, $intLangId)
	{
		...
		if ($objEavEntityValueTableRowSet->count() <= 0) {
			...
			$arrEntityValueData[Bf_Eav_Db_EntitiesValues::COL_IS_DELETED] = 0;
			...
			if ($objValueRow->{Bf_Eav_Db_EntitiesValues::COL_IS_DELETED}) {
				$objValueRow->{Bf_Eav_Db_EntitiesValues::COL_IS_DELETED} = 0;
				$objValueRow->save();
			}
		}
		return TRUE;
	...
	*/
	public function getAttrEditForm ($intAttrId, $mixAttrType)
	{
		...
		// Lansweeper Name
		$objLansweeperCode = new Zend_Form_Element_Text(Bf_Eav_Db_Attributes::COL_LANSWEEPER_CODE);
		$objLansweeperCode->setLabel('LBL_LANSWEEPER_CODE');
		$objLansweeperCode->addValidator('Db_NoRecordExists', TRUE, array('table' => Bf_Eav_Db_Attributes::TBL_NAME, 'field' => Bf_Eav_Db_Attributes::COL_LANSWEEPER_CODE));
		...
		$objAttrUserEdit = new Zend_Form_Element_Checkbox(Bf_Eav_Db_Attributes::COL_IS_USER_CAN_EDIT);
		$objAttrUserEdit->setLabel('LBL_EAV_ATTR_IS_USER_CAN_EDIT')
		->setAttrib('id', Bf_Eav_Db_Attributes::COL_IS_USER_CAN_EDIT)
		->setRequired(FALSE);
		...
		// Group Them
		$objForm->addDisplayGroup(array($objAttrId, $objAttrCode, $objLansweeperCode, $objAttrDesc, $objAttrUnits, $objAttrShowList,$objAttrUserEdit, $objAttrType), 'attr_spec', array('legend' => "LBL_ATTR_SPEC"));
	...
	public function setGridCol($objAttrListRow, $objGrid, $strSearchValue="")
	{
		$arrSearchOptions = array('defaultValue' => $strSearchValue);

		$objColumn = new Ingot_JQuery_JqGrid_Column(
			str_replace( ' ', '_', $objAttrListRow->{Bf_Eav_Db_Attributes::COL_ATTR_CODE} ),
			array(
				'sortable' => false,
				'useHaving' => true,
				'customField' => Bf_Catalog_Models_Catalog::COL_ATTR_DATA,
				'unionPart' => 1,
				'autowidth' => true,
				'shrinkToFit' => false,
		...
		$objDecorator = new Ingot_JQuery_JqGrid_Column_Decorator_Eav(
			$objColumn,
			array(
				'intAttrId' => $objAttrListRow->{Bf_Eav_Db_Attributes::COL_ID_ATTR},
				'eavValueType' => $objAttrListRow->{Bf_Eav_Db_Attributes::COL_VALUE_TYPE},
			)
		);
